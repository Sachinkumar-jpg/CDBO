...
from Proposed_CDBO import CDBO
...
class Hetnet:
    def __init__(self, N_D2D, N_CU, D2D_dis, SimulationRegion, W, PLfactor, PL_k,
        ...
        print("\n........ User Grouping or User mode Selection ........")

        print("FCM Running...")
        self.d_U, self.r_U, self.d_ind, \
            self.r_ind = FCM.cluster(self.N_CU, self.cluster)   # FCM clustering algorithm called from here.
        print("User Grouping Done !!!\n")
    ...
    def reset(self):
        ...
        # ------------ In Dedicated mode user power allocation is based on Dinkelbach-like like algorithm ------------
        print("........ Dedicated User Mode:\tD2D Power allocation ........")
        self.power_levels1 = []
        print("Dinkelbach algorithm Running...")
        ...
        # ------------ In Reuse mode user power allocation is based on Proposed CBDO------------
        print("\n........ Reuse user Mode:\tD2D Resource allocation ........")
        r_X, r_Y = [], []
        for i in range(0, len(self.r_U)):
            r_X.append(CU_Position_x[self.r_ind[i]])
            r_Y.append(CU_Position_y[self.r_ind[i]])

        print("SMS algorithm running...")
        self.power_levels2 = sms.resource_allocate(self.r_U, r_X, r_Y, self.D2D_tr_Power_levels)

        print("Proposed CBDO Running...")
        self.throughput = np.arange(0, self.D2D_tr_Power_levels + 1)
        for i in range(0, len(self.r_U)):
            self.power_levels2[i] = CDBO.caviar_DBO(self.D2D_tr_Power_max, self.N_D2D,
                                                    self.power_levels2[i], self.throughput[i])       # Proposed CDBO
