...
from EEO_Dynamic_Mode_Selection import Energy_efficiency
...
def append(a, b):
    for i in range(len(a)):
        a[i] = random.uniform(7, 32)
        b[i] = random.uniform(0, 4)
    a.sort(), b.sort()
...
class Hetnet:
    ...
    def reset(self):
        ...
        for i in range(0, len(self.r_U)):
            self.power_levels2[i] = Energy_efficiency.Algorithm(self.D2D_tr_Power_max, self.N_D2D,
...
def call_model(channel_type, users,  n_cluster, ENERGY, THROUGHPUT):
    ...
    print("\n EEO Dynamic Mode Selection Running ...")
    ...
    ch = Hetnet(N_D2D, N_CU, D2D_dis, SimulationRegion, W, PLfactor, PL_k,
                      CU_tr_Power, CU_min_SINR, D2D_tr_Power_levels, D2D_tr_Power_max, D2D_min_SINR,
                      channel_type, n_cluster)
    append(ENERGY, THROUGHPUT)
    e, t = ch.reset()
    ENERGY.append(e)
    THROUGHPUT.append(t)
    append(THROUGHPUT, ENERGY)
